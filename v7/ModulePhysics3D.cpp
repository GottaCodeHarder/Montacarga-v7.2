...
update_status ModulePhysics3D::Update(float dt)
{
	...
	if(debug == true)
	{
		...
		if(App->input->GetKey(SDL_SCANCODE_1) == KEY_DOWN)
		{
			...
			float force = 30.0f;
			AddPallet(s)->Push(-(App->camera->Z.x * force), -(App->camera->Z.y * force), -(App->camera->Z.z * force));
		}
	}
...
// ---------------------------------------------------------
PhysBody3D* ModulePhysics3D::AddPallet(const Cube& cube, float mass)
{
	float PI = 3.141592;
	...
	btCollisionShape* pallet_z = new btBoxShape(btVector3(0.2f, 0.4f, 4.0f));
	shapes.add(pallet_z);

	btCollisionShape* pallet_x = new btBoxShape(btVector3(4.0f, 0.1f, 0.4f));
	shapes.add(pallet_x);

	btCollisionShape* box = new btBoxShape(btVector3(4, 4, 4));
	shapes.add(box);

	trans.setOrigin(btVector3(-1.9*2, 0.25*2,0));
	comShape->addChildShape(trans, pallet_z);
	trans.setOrigin(btVector3(1.9*2, 0.25*2,0));
	comShape->addChildShape(trans, pallet_z);
	trans.setOrigin(btVector3( -0*2, 0.25*2,0));
	comShape->addChildShape(trans, pallet_z);
	
	trans.setOrigin(btVector3(0, 5, 0));
	comShape->addChildShape(trans, box);

	for (int i = 0; i < 7; ++i)
	{
		trans.setOrigin(btVector3(0,0.5*2,(-1.8+(0.6*i))*2));
		comShape->addChildShape(trans, pallet_x);
	}

	for (int i = 0; i < 3; i++)
	{
		trans.setOrigin(btVector3(0, 0,(-1.8 +(0.6*i * 3)) * 2));
		comShape->addChildShape(trans, pallet_x);
	}
	...
	btDefaultMotionState* myMotionState = new btDefaultMotionState(startTransform);
	motions.add(myMotionState);
	btRigidBody::btRigidBodyConstructionInfo rbInfo(mass, myMotionState, comShape, localInertia);
...
PhysBody3D* ModulePhysics3D::Addelevators(const Cube& cube, float mass)
{
	btCompoundShape* comShape = new btCompoundShape();
	shapes.add(comShape);
	...
	btCollisionShape* elev = new btBoxShape(btVector3(0.1, 0.4f, 4.5f));
	shapes.add(elev);

	trans.setOrigin(btVector3(1.0f,1.8f,0.0f));
	comShape->addChildShape(trans, elev);
